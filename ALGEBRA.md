...
This is language multiplication. If we extended from finite spaces to all subsets of `Σ*`, then union as join and language multiplication would form the usual powerset quantale of a monoid. Zippy intentionally lives in the finite fragment, but the analogy remains useful; the semiring laws are spelled out in §3.1.
...
Composition's two division adjoints — the left and right residuals of the residuated structure — are deliberately omitted from the language: their universally-quantified membership tests resist every fast-path mechanism the engine has. See `residuals.md` for their definitions and the reasoning.
...
`TailsIntersection` adds a small but useful universal quantifier: restricting a key-value space to selected keys and intersecting the tails under those keys computes the values common to *every* selected key,

```text
{ v | ∀k ∈ selectedKeys. k.v ∈ keys }
```

That is not a full higher-order quantifier over arbitrary formulas, but it is exactly the universal query that arises in trie-shaped key/value data: common tags, common capabilities, common neighbors, common attributes. A worked example is in §4.4.
...
Here `Σ*` is the set of all suffixes, and `P · Σ*` means “all words whose prefix is in `P`.” The reference evaluator writes this as a scan (§4); the trie executors instead descend to each prefix in `P` and enumerate the subtree below it. That changes the practical cost from “scan all of `X`” to “walk the selected prefixes and emit the result” — ignoring prefix lookup cost and sharing details, work proportional to the size of the answer rather than the size of the database.
...
This makes restriction and iteration output-sensitive for the common cases. The implementation carries the algebra across three executors: the set-of-paths reference (the direct semantics of §4), an eager trie executor implementing the table above structurally, and a zipper executor that navigates the trie by single-key moves without materializing intermediate results. The three are proven observationally equivalent (§5.9); the notation records enough structure for all of them to avoid treating paths as opaque strings.

## 4. The operations

The direct evaluator is almost the denotational semantics — and it serves as the reference every optimized executor is verified against (§5.9). The important cases fit on one page:
...
The empty-input case is pinned down by the formalization: a space with no heads (empty, or `{ε}`) has tails-intersection `∅` — the machine-checked membership characterization forced this choice, and stating it caught a wrong `{ε}`-source rule in an earlier rewrite system.
...
### 5.8 The supercompiler

The laws above are *transient reductions*: they simplify a configuration but never unfold a recursive routine. A full positive supercompiler adds the three classical moves on a process tree, specialized to the set algebra:

- **Driving** — `reduce` a configuration with the laws above to a normal form, then *unfold* the outermost routine call, **including self-calls** (the one thing `Routine.optimized` refuses to do).
- **Folding** — when a driven configuration is an *instance* of a previously seen one, tie the knot into a residual recursive routine.
- **Generalization under a whistle** — a homeomorphic embedding on the `Space`/`Path` term order; when an ancestor embeds in a descendant without being an instance, abstract the difference by most-specific generalization so the process tree stays finite.

The headline outcome is recursion *transformed*, not merely simplified: supercompiling a semi-naive transitive-closure solver against a static graph fuses the graph in as a literal and generalizes the accumulator/frontier into a reusable loop whose size is independent of the graph. The embedding deliberately treats any literal as one atom, so a growing static accumulator trips the whistle; with structural embedding instead, static recursion is fully evaluated. Substitution is capture-avoiding, grounded host functions are opaque operations keyed by identity, and generated names can never collide with user names.

### 5.9 Machine-checked laws and executors

The laws are not asserted; they are certified, and so are the executors that use them.

- **Per-law certificates.** Every law in this paper has a universal certificate: a small
  ∀-statement over the set-of-paths denotation, discharged by two independent first-order
  provers. The corpus covers the semiring core, the wrap/unwrap/composition monoid actions,
  restriction and raffination (including the partition `(x\|y) ∪ (x<|y) = x`), the tails/head
  quotient family, and the iteration laws (invariant hoisting, fusion). Structural induction is
  never smuggled in: the few lemmas that need it carry explicit induction-schema instances, so
  plain first-order provers finish base and step, and every rewrite rule is mechanically tied to
  a live certificate.
- **Executor equivalence.** For each program, the optimised term, the zipper program, and the
  compiled operation graph are proved observationally equivalent to the reference semantics —
  equationally in an e-graph engine, and denotationally in the provers — on the case studies
  below (plus n-queens), with data-agnostic (free-input) variants. Where the optimiser rewrites
  a subterm, the pair is verified as an instance of the certified law set by syntactic replay,
  so program-level proofs reduce to the law corpus.
- **Termination.** The Datalog, reachability, and SCC recursions terminate by machine-checked
  measure arguments: strict growth inside a finite universe shrinks the complement, and a
  lexicographic measure handles semi-naive evaluation.
...
The examples suggested a practical direction, and the implementation has since followed it: spaces are stored as (hash-consed, interned) tries; restriction compiles to guided traversal; `TailsUnion` lowers to iteration; invariant prefixes hoist; literals unroll; recursive rules run incremental frontiers; and a zipper executor navigates by single-key moves without materializing intermediate spaces. A positive supercompiler applies the law set to whole programs (§5.8), and the entire stack — laws, rewrite systems, and executors — is machine-checked against the reference semantics (§5.9). The surface language remains a small algebra of path spaces; the implementation exploits exactly the structure the notation reveals.
