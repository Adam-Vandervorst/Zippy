...
/** tasks.md D3 — PUZZLE15 AS A FIRST-CLASS STRESS THEOREM.
 *
...
class Puzzle15Check extends FunSuite:
  ...
  test("INVARIANTS: every board of the first BFS levels is a legal state, every successor a legal move, the counts match the independent oracle") {
    ...
    sb ++= "# PUZZLE15 EXPANSION (tasks.md D3) — the BFS levels the MORKL program produced; re-derived by scripts/check_puzzle15.py\n"
  ...
  test("ARTIFACTS: the certificate and the derivation report") {
    val sb = new StringBuilder
    sb ++= "# PUZZLE15 CERTIFICATE (tasks.md D3) — one expansion priced under four declarations on four backends; re-derived by scripts/check_puzzle15.py\n"
