...
/** ==============================================================================================
 *  DECISION CASES (tasks.md B3): choices a scalar estimator cannot make safely.
 *
...
object Decisions:
  ...
  final case class Case(
      name: String,
      /** the transformation family tasks.md B3 names */
      family: String,
  ...
  val indexHeader: String =
    "# DECISION CASES (tasks.md B3) — one row per case; generated by DecisionsCheck, replayed by scripts/check_selection.py\n" +
  ...
  def document(outcomes: Vector[Outcome]): String =
    ...
    sb ++= "<!-- GENERATED by `ZIPPY_REGENERATE=1 sbt --server 'testOnly morkl.DecisionsCheck'` (tasks.md B3); a verify run fails on drift. -->\n\n"
    sb ++= "Every case below is one symbolic program under a stated input precondition. Its residual alternatives are\n"
    sb ++= "produced by the supercompiler under several law tables and unroll depths (`Alternatives`, tasks.md B1), each\n"
    sb ++= "with a typed proof trace that has CLOSED (every law step certified, every unfold/fold mechanized) and a\n"
    sb ++= "resource certificate per backend (`CostSem`, A4/A5). The choice is made by `Pareto.select` (B2): admission,\n"
    ...
    sb ++= "The trie, zipper and graph intervals are the must/may envelopes of the abstract interpretation (A4); they are\n"
