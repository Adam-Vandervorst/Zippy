...
%
%
% THIS FILE: THE EQUALITY, from the two inclusions by ANTISYMMETRY.  MEASURED:
% each inclusion closes on its own in seconds from the same axiom base, while
% the equality goal did not close in 90 s — saturation attacks an equational
% goal by trying to rewrite `iter(cup(a0,b0),f0)`, not by looking for the two
% inclusions.  Splitting the theorem the way the mathematics splits it, and
% importing the halves as staged lemmas (`iteration_split_sub.p`,
% `iteration_split_sup.p`, both PROVED), makes the last step trivial and keeps
% every part of the argument machine-checked.
%
...
tff(split_sub, axiom, sub(iter(cup(a0,b0), f0), cup(iter(a0,f0), iter(b0,f0))) ).
tff(split_sup, axiom, sub(cup(iter(a0,f0), iter(b0,f0)), iter(cup(a0,b0), f0)) ).

tff(iteration_split, conjecture,
    iter(cup(a0,b0), f0) = cup(iter(a0,f0), iter(b0,f0)) ).
