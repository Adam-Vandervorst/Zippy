; THREE-WAY EQUIVALENCE (subtraction): the ZIPPER observation, the SET-OF-PATHS denotation, and the EAGER
...
; ZIPPER observation of the virtual subtraction cursor, following the movement rules verbatim
(define-fun-rec zipobs ((a Trie) (b Trie) (p Path)) Bool
  (match p ((nil (and (term a) (not (term b))))
...
(define-fun PP ((p Path)) Bool (forall ((a Trie) (b Trie))
  (and (= (zipobs a b p) (and (mem a p) (not (mem b p))))          ; zipper = set of paths
       (= (mem (f a b) p) (and (mem a p) (not (mem b p)))))))      ; eager trie = set of paths
