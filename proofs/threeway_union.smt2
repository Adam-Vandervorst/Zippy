; THREE-WAY EQUIVALENCE (union): the ZIPPER observation, the SET-OF-PATHS denotation, and the EAGER
...
; ZIPPER observation of the virtual union cursor, following the movement rules verbatim
(define-fun-rec zipobs ((a Trie) (b Trie) (p Path)) Bool
  (match p ((nil (or (term a) (term b)))
...
(define-fun PP ((p Path)) Bool (forall ((a Trie) (b Trie))
  (and (= (zipobs a b p) (or (mem a p) (mem b p)))          ; zipper = set of paths
       (= (mem (f a b) p) (or (mem a p) (mem b p))))))      ; eager trie = set of paths
