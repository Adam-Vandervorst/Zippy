; SPATIAL-TYPE CERTIFICATE (concretion) — RAFFINATION IS EXACT ON SHORT CLASSES: a path shorter than every prefix is always kept
...
(assert (not (=> (forall ((q Path)) (=> (B q) (>= (len q) m)))
       (forall ((p Path)) (=> (and (A p) (< (len p) m)) (MRaff p))))))
(check-sat)
