; SPATIAL-TYPE CERTIFICATE (concretion) — SELF-RESTRICTION IS THE IDENTITY (x <| x = x) and SELF-RAFFINATION IS EMPTY — every path is its own prefix
...
(assert (forall ((p Path)) (isPrefix p p)))   ; sp_isprefix_refl
(assert (not (and (forall ((p Path)) (= (and (A p) (exists ((q Path)) (and (A q) (isPrefix q p)))) (A p)))
