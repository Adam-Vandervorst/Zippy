; SPATIAL-TYPE CERTIFICATE (concretion) — SUBSUMPTION TIGHTENING: y ⊆ x ⇒ x∪y = x, x∩y = y, and x∖y is exactly the complement in x
...
(assert (not (=> (forall ((p Path)) (=> (B p) (A p)))
       (and (forall ((p Path)) (= (MUnion p) (A p)))
            (forall ((p Path)) (= (MInter p) (B p)))
            (forall ((p Path)) (= (MSub p) (and (A p) (not (B p)))))))))
(check-sat)
