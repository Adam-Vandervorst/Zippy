; SPATIAL-TYPE CERTIFICATE (concretion) — DISJOINT CYLINDERS: bodies keyed by DISTINCT group heads cannot share a path, so a keyed iteration's per-group results are pairwise disjoint (its size is an exact SUM)
...
(assert (not (forall ((h1 Int) (h2 Int) (p Path) (q Path))
       (=> (not (= h1 h2)) (not (= (cons h1 p) (cons h2 q)))))))
(check-sat)
