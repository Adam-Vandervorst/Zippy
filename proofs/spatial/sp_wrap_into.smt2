; SPATIAL-TYPE CERTIFICATE (concretion) — WRAP maps each source path INJECTIVELY to one |w|-longer path (the class shift, ⊇ direction)
...
(assert (not (and (forall ((q Path)) (=> (A q) (and (MWrap (append w q)) (= (len (append w q)) (+ (len w) (len q))))))
