; SPATIAL-TYPE CERTIFICATE (concretion) — ITERATION GROUP KEYS HAVE LENGTH EXACTLY 1 (the head-binder length hint the analysis relies on)
...
(assert (not (and (forall ((p Path)) (=> (MHead p) (= (len p) 1)))
       (forall ((h Int) (t Path)) (=> (A (cons h t)) (MHead (cons h nil)))))))
(check-sat)
