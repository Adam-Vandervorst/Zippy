; SPATIAL-TYPE CERTIFICATE (concretion) — PARTITION: x<|y and x\|y are disjoint and cover x (licenses the exact-sum union)
...
(assert (not (and (forall ((p Path)) (= (or (MRestrict p) (MRaff p)) (A p)))
       (forall ((p Path)) (not (and (MRestrict p) (MRaff p)))))))
(check-sat)
