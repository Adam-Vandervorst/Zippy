; SPATIAL-TYPE CERTIFICATE (concretion) — LENGTH-DISJOINT MEET IS EMPTY: classes at different lengths cannot intersect
...
(assert (not (=> (and (forall ((p Path)) (=> (A p) (= (len p) m)))
            (forall ((p Path)) (=> (B p) (not (= (len p) m)))))
       (forall ((p Path)) (not (MInter p))))))
(check-sat)
