...
(declare-fun AB (Path) Bool)
(declare-fun BC (Path) Bool)
(assert (forall ((p Path)) (= (AB p) (exists ((uu Path) (vv Path)) (and (= p (append uu vv)) (A uu) (B vv))))))
(assert (forall ((p Path)) (= (BC p) (exists ((uu Path) (vv Path)) (and (= p (append uu vv)) (B uu) (C vv))))))
...
(assert (not (forall ((p Path)) (= (exists ((u Path) (v Path)) (and (= p (append u v)) (AB u) (C v))) (exists ((u Path) (v Path)) (and (= p (append u v)) (A u) (BC v)))))))
(check-sat)
