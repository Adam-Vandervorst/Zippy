; LAW CERTIFICATE — wrap w A = {w}·A (wrap IS composition with a singleton)
...
(assert (not (forall ((p Path)) (= (exists ((qq Path)) (and (= p (append w qq)) (A qq))) (exists ((uu Path) (vv Path)) (and (= p (append uu vv)) (= uu w) (A vv)))))))
(check-sat)
