; LAW CERTIFICATE — restriction: ∅ cases, ε-prefix identity, ∪-distribution (prefixes), singleton = wrap∘unwrap
...
(assert (not (and
  (forall ((p Path)) (= (and false (exists ((qq Path)) (and (B qq) (isPrefix qq p)))) false))
  (forall ((p Path)) (= (and (A p) (exists ((qq Path)) (and false (isPrefix qq p)))) false))
  (forall ((p Path)) (= (and (A p) (exists ((qq Path)) (and (= qq nil) (isPrefix qq p)))) (A p)))
  ...
  (forall ((p Path)) (= (and (A p) (exists ((qq Path)) (and (= qq w) (isPrefix qq p)))) (exists ((qq Path)) (and (= p (append w qq)) (A (append w qq)))))))))
(check-sat)
