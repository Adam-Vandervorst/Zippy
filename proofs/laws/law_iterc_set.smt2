; LAW CERTIFICATE — IterC (invariant body over heads): ∅ source, ε-singleton source (NO heads: ∅ — the ε case the rule split fixed), non-ε singleton, ∪-distribution
...
(assert (not (and
  (forall ((p Path)) (= (and (exists ((hh Int) (tt Path)) false) (B p)) false))
  (forall ((p Path)) (= (and (exists ((hh Int) (tt Path)) (= (cons hh tt) nil)) (B p)) false))
  (forall ((h Int) (t Path)) (forall ((p Path)) (= (and (exists ((hh Int) (tt Path)) (= (cons hh tt) (cons h t))) (B p)) (B p))))
  (forall ((p Path)) (= (and (exists ((hh Int) (tt Path)) (or (A (cons hh tt)) (C (cons hh tt)))) (B p)) (or (and (exists ((hh Int) (tt Path)) (A (cons hh tt))) (B p)) (and (exists ((hh Int) (tt Path)) (C (cons hh tt))) (B p))))))))
(check-sat)
