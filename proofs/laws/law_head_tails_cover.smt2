; LAW CERTIFICATE — head(A)·tails∪(A) ⊇ A on ε-free A (a SUBSET statement, as a ∀-implication)
...
(assert (not (forall ((p Path)) (=> (and (A p) (distinct p nil)) (exists ((uu Path) (vv Path)) (and (= p (append uu vv)) (exists ((hh Int)) (and (= uu (cons hh nil)) (exists ((tt Path)) (A (cons hh tt))))) (exists ((hh Int)) (A (cons hh vv)))))))))
(check-sat)
