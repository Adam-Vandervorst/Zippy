; AUTO-GENERATED — TRUE fixpoint: init absorbed into the step
...
(define-fun s_5 ((p Path)) Bool (or (in_src p) (or (= p (cons 0 nil)))))
(assert (forall ((zq Path)) (=> (s_5 zq) (fix_4 zq))))
(assert (forall ((zq Path)) (=> (or (fix_4 zq) (or (= zq (cons 0 nil)))) (fix_4 zq))))
...
; DERIVED-FROM: proofs/lean/Zippy/Positive.lean#Zippy.Space.fixpoint_is_lfp
(assert (=> (and (forall ((zr Path)) (=> (s_5 zr) (fix_2 zr))) (forall ((zr Path)) (=> (or (fix_2 zr) (or (= zr (cons 0 nil)))) (fix_2 zr)))) (forall ((zq Path)) (=> (fix_4 zq) (fix_2 zq)))))
