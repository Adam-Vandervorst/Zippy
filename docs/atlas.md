...
| Cost | `SpatialCost.analyze` | — | `CostInterval` over `Sym`, per `Backend` |
...
| `Sym` / `Cost` / `CostInterval` / `Meas` / `Rel` / `CostModel` | `SpatialCost.scala` | The symbolic cost algebra (`1 < log n < n < n log n < n² < 2ⁿ`) and the backend instances (`ReferenceCost`, `TrieCostModel`, `GraphCost`, `ZipperCost`, plus the diagnostic `NaiveTrieCostModel`), each transfer returning a lower/upper `CostInterval` over `work`/`alloc`/`rounds`/`touch`. `analyze` is the entry point; `FrontierCensus` publishes how much of a program was frontier-driven. |
...
| `SpatialCostCheck` | The `Sym` algebra, `dominates` soundness, per-operator/per-backend cost shape, recurrence closing, and the per-operator width table. |
| `SpatialEventsCheck` | The event vocabulary (every event has an emitter and exactly one component), hand-computed hook counts per executor, and the calibration tables. |
| `SpatialScaleCheck` | `ProductRequirement` (the width/error/slope tiers) and the correlated generator families over geometric ladders. |
