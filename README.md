...
| [`docs/DECISIONS.md`](docs/DECISIONS.md) | tasks.md B1–B3 (GENERATED by `DecisionsCheck`): the decision cases — residual alternatives with closed traces and per-backend certificates, the certified Pareto selection under a declared objective, every alternative's counted run beside its predicted interval, and the two scalar predictors (rewrite count, output cardinality) evaluated on the same frontier; machine-readable twin `proofs/decisions/`. |
| [`docs/ACCEPTANCE.md`](docs/ACCEPTANCE.md) | tasks.md E3 (GENERATED by `scripts/gen_acceptance.py`): one row per tasks.md item — GREEN / CONDITIONAL / UNSOUND / SOUND-BUT-NOT-USEFUL / UNMEASURED — derived from the acceptance sentence, the gates `docs/ACCEPTANCE_MAP.tsv` names and their recorded results, the proof closures and the committed evidence; `--check` fails when the document disagrees with the evidence. |
| [`proofs/puzzle15/REPORT.md`](proofs/puzzle15/REPORT.md) | tasks.md D3 (GENERATED by `Puzzle15Check`): the puzzle15 derivation report — the state-space theorems (`proofs/lean/Zippy/Puzzle15.lean`), the checked expansion, the certificates per declaration and backend, the usefulness verdicts, where the symbolic declarations lose. |
...
python3 scripts/check_selection.py       # every committed selection certificate (proofs/decisions)
                                         ...
                                         # interval dominance, the declared tie rule (B2/B3)
python3 scripts/check_coverage.py        # structural coverage (D1): every feature row of
...
python3 scripts/check_puzzle15.py        # the puzzle15 stress theorem (D3): the committed expansion is a
...
python3 scripts/gen_acceptance.py --check# docs/ACCEPTANCE.md agrees with the recorded gate results, the
                                         # closures and the committed evidence (E3); `--selftest` shows
